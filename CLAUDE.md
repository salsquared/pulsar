...
- `npm run start` — Hono server on **port 3103** (the only environment). Loads `.env` and runs the built `dist/index.js`; PM2's `pulsar` entry owns the lifecycle, so use `pm2 reload pulsar` after a rebuild.
- `npm run dev` — same server via tsx, no build step. For local iteration outside PM2.
- `npm run dev:watch` — same as `dev` with `--watch`. Standalone only; do not run inside PM2.
- `npm run build` — compiles TypeScript to `dist/`.
- `npm run lint` — ESLint + TypeScript type-check.
- `npx prisma migrate dev` / `npx prisma generate` — schema at `prisma/schema.prisma` (SQLite, single DB at `prisma/pulsar.db`).
- `npx tsx src/index.ts` — run the entry point directly without building.
...
There is one SQLite file (`prisma/pulsar.db`) selected by `DATABASE_URL` in `.env`. The five core models are: `Asset` (registry), `PriceTick` (raw ticks), `DailySummary` (pre-aggregated for chart queries), `MacroSeries` (FRED/macro indicators), and `IngestJob` (run log). All timestamps are stored as UTC.
...
`.env` is gitignored. Copy the committed template and fill in values:

```
cp .env.example .env
```

It contains `DATABASE_URL`, `PULSAR_INTERNAL_TOKEN`, and all external API keys. `.env.example` is committed and documents the required keys.
